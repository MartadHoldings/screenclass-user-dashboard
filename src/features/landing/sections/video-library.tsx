...
export const VideoLibrary = () => {
  return (
    <section className="grid h-screen w-full place-items-center bg-bg-index-three bg-cover bg-center bg-no-repeat px-2 font-poppins sm:px-8 lg:px-14">
      <div className="w-full border-2 border-black">
        ...
        <p className="text-center text-[10px] font-semibold text-black lg:text-2xl">
          ...
          <span className="text-[8px] font-medium text-[#BC3508] md:text-sm lg:text-xl">
            10,000
          </span>{" "}
          students have watched these
        </p>
      </div>
      <div className="flex w-full flex-wrap gap-x-4 gap-y-6 border-2 border-black">
        {data.map((item, index) => (
          <div className="h-fit max-w-[283px] rounded-sm bg-white" key={index}>
            <VideoCard {...item} />
          </div>
        ))}
      </div>
